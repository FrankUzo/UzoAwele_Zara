...
const ShopConextProvider = (props) => {
  ...
  console.log("sizeQuantity sizeQuantity sizeQuantity:", sizeQuantity);
  ...
  function addToCart(cartitem, size) {
    ...
    if (alreadyInCartIndex != -1) {
      ...
      console.log("cartSizeItem cartSizeItem", cartSizeItem);

      if (cartSizeItem != null) {
        item.selectedSize.push({ title: size, qty: item.selectedQty });
  ...
  const updateQuantity = async (item, quantity, size) => {
    console.log("updateQuantity updateQuantity item: ", item);
    console.log("updateQuantity updateQuantity size: ", size);
    ...
    let newCartItem = localStoragecartItems.find((element) => {
      console.log("element Id element Id element:", element);
      ...
      if (size == null) {
        ...
        console.log(
          "updateQuantity updateQuantit Else Logic Ran element.selectedSize: ",
          element.selectedSize
        );
        let cartSizeItem = element.selectedSize.find((eachSizeObject) => {
          return eachSizeObject.title === size.title;
        });

        if (cartSizeItem != null) {
          cartSizeItem["qty"] = quantity;
          let prdtIndex = element.selectedSize.findIndex(
            (obj) => obj.title === size.title
          );

          element.selectedSize[prdtIndex] = cartSizeItem;
          // console.log("cartSizeItem found cartSizeItem: ", cartSizeItem);
          // return cartSizeItem; //._id == item._id;
        }
      }
    });

    console.log("newCartItem newCartItem: ", newCartItem);
