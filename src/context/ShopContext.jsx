...
const ShopConextProvider = (props) => {
  ...
  const [className, setClassName] = useState("grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6");
  ...
  const updateQuantity = async (item, quantity, sizeObject) => {
     ...
     */
    if (quantity == 0 && sizeObject != null) {
      deleteSizeItemFromCart(itemFromCartItems, sizeObject, localStoragecartItems, elementIndex);
      return;
    } 
  ...
  function deleteSizeItemFromCart(itemFromCartItems, sizeObject, localStoragecartItems, elementIndex) {
      let deleteProductIndex = itemFromCartItems.selectedSize.findIndex(
        (obj) => obj.title === sizeObject.title
      );
     
      var newSizesArray = [];
      for (let index = 0; index < itemFromCartItems.selectedSize.length; index++) {
